...
class OmniGPUModelRunner(PrefixCacheRunnerMixin, GPUModelRunner):
    ...
    def _build_model_kwargs_extra(self) -> dict:
        ...
        try:
            ...
            # Models that consume this list in compute_logits cache it during
            # forward, which a replayed FULL decode graph skips. Refresh the
            # cache here, outside any capture, so the constraint logic always
            # sees the current batch order (e.g. after batch condensation).
